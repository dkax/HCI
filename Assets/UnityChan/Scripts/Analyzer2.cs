...
class Analyzer2 : AbstractAnalyzer {
    ...
    override public bool Analyze(Frame frame) {
        ...
        if (count == 0) {
            ...
            if (frame.Hands[0].IsLeft) {//场景中有左手
        ...
        double dis = 0.0;
        dis = CalDis(PalmPosition[indexNow], PalmPosition[(indexNow + 1) % 2]);//移动距离
        Vector vec = PalmPosition[indexNow] - PalmPosition[(indexNow + 1) % 2];//移动向量
